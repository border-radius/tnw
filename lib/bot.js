    ...
    switch (commandName) {
      ...
      case 'help':
        return [
          ...
          '/bl [username] - add/remove user from blacklist or just show blacklisted users'
        ].join('\n')
      case 'stop':
      ...
      case 'bl':
        ...
        return updateBlackList.then(() => {
          return bnw({
            ...
            identity.blacklist = response.blacklist.filter(item => item[0] == 'user').map(item => item[1])
